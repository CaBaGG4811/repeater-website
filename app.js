...
function toast(m) {
    ...
    clearTimeout(t._);
    t._ = setTimeout(function() { t.classList.remove('active'); }, 2800);
}
...
function showScreen(n) {
    ...
    var b = document.querySelector('[data-screen="' + n + '"]');
    if (b) b.classList.add('sidebar__btn--active');
    if (n === 'dashboard') renderDash();
    if (n === 'topics') renderTopics();
    if (n === 'posts') renderPosts();
    if (n === 'stats') renderStats();
    if (n === 'settings') renderSet();
    var sa = document.getElementById('scroll-area');
    if (sa) sa.scrollTop = 0;
    setTimeout(initReveal, 100);
}

document.querySelectorAll('.sidebar__btn').forEach(function(b) {
    b.addEventListener('click', function() { showScreen(b.dataset.screen); });
});
document.querySelectorAll('.sidebar__settings').forEach(function(b) {
    b.addEventListener('click', function() { showScreen(b.dataset.screen); });
});
...
function renderBubbles() {
    ...
    var ba = document.getElementById('bubbles-area');
    if (ba) {
        ba.innerHTML = bs.map(function(b, i) {
...
function renderCal() {
    ...
    var cm = document.getElementById('cal-month');
    if (cm) cm.textContent = mn[m] + ' ' + y;
    var dd = {};
    topics.forEach(function(t) {
        if (!t.done) {
            var d = new Date(t.nr);
            if (d.getMonth() === m && d.getFullYear() === y) dd[d.getDate()] = true;
        }
    });
    ...
    for (var d = 1; d <= dim; d++) {
        var c = 'calendar__day';
        if (d === td) c += ' calendar__day--today';
        else if (dd[d]) c += ' calendar__day--due';
        h += '<div class="' + c + '">' + d + '</div>';
    }
    cg.innerHTML = h;
}
...
function renderSteps() {
    ...
    var sd = document.getElementById('steps-done');
    var st = document.getElementById('steps-total');
    if (sd) sd.textContent = don;
...
function renderWeekly() {
    ...
    topics.forEach(function(t) {
        if (t.rc > 0) {
            var d = new Date(t.dateAdded);
            dc[(d.getDay() + 6) % 7] += (t.rc || 1);
        }
    });
    var max = Math.max.apply(null, dc.concat([1]));
    var colors = ['#FF6B6B','#FFB800','#34C759','#4DABF7','#A78BFA','#F97316','#EC4899'];
    var wb = document.getElementById('weekly-bars');
    if (!wb) return;
    wb.innerHTML = WD.map(function(d, i) {
...
function renderTopics() {
    ...
    var ts = document.getElementById('topics-subtitle');
    if (ts) ts.textContent = topics.length + ' тем, ' + due + ' требуют повторения';
    var list = document.getElementById('topics-list'), emp = document.getElementById('empty-topics');
    if (!list) return;
    if (!f.length) { list.innerHTML = ''; if (emp) emp.style.display = 'block'; return; }
...
function renderChips() {
    ...
    if (tc) {
        tc.innerHTML = CATS.map(function(c) {
            return '<button class="chip' + (selCat === c ? ' chip--active' : '') + '" onclick="selCat=\'' + c + '\';renderTopics()">' + c + '</button>';
        }).join('');
    }
}
...
function reviewTopic(id) {
    ...
    t.interval = stageInt(t.st);
    t.nr = Date.now() + t.interval * 864e5;
    t.rc = (t.rc || 0) + 1;
    save();
    renderTopics();
    renderDash();
...
function openAddTopicDialog() {
    var ti = document.getElementById('topic-title-input');
    if (ti) ti.value = '';
    selModCat = 'Другое';
    renderModChips();
    document.getElementById('modal-overlay').classList.add('active');
    document.getElementById('add-topic-modal').classList.add('active');
    setTimeout(function() { var ti = document.getElementById('topic-title-input'); if (ti) ti.focus(); }, 100);
}
...
function renderModChips() {
    ...
    var mc = document.getElementById('modal-category-chips');
    if (mc) {
        mc.innerHTML = cs.map(function(c) {
            return '<button class="chip' + (selModCat === c ? ' chip--active' : '') + '" onclick="selModCat=\'' + c + '\';renderModChips()">' + c + '</button>';
        }).join('');
    }
}

document.getElementById('topic-title-input').addEventListener('input', function() {
    var a = catTitle(this.value);
    if (a !== 'Другое') { selModCat = a; renderModChips(); }
});

document.getElementById('topic-title-input').addEventListener('keydown', function(e) {
...
function renderPosts() {
    var cs = ['Все'];
    POSTS.forEach(function(p) { if (cs.indexOf(p.cat) === -1) cs.push(p.cat); });
    var pc = document.getElementById('post-chips');
    if (pc) {
        pc.innerHTML = cs.map(function(c) {
            return '<button class="chip' + (selPostCat === c ? ' chip--active' : '') + '" onclick="selPostCat=\'' + c + '\';renderPosts()">' + c + '</button>';
        }).join('');
    }
    var f = selPostCat === 'Все' ? POSTS : POSTS.filter(function(p) { return p.cat === selPostCat; });
    var pl = document.getElementById('posts-list');
    if (pl) {
        pl.innerHTML = f.map(function(p, i) {
...
function handleAct(type, data) {
    if (type === 'create_topic') {
        ...
        document.getElementById('add-topic-modal').classList.add('active');
        setTimeout(function() { var ti = document.getElementById('topic-title-input'); if (ti) ti.focus(); }, 100);
    } else if (type === 'open_timer') {
        toast('Pomodoro: 25 мин - 5 мин отдыха - 4 цикла');
...
function renderStats() {
    ...
    var sl = document.getElementById('stats-layout');
    if (!sl) return;
    sl.innerHTML =
...
function drawChart(data) {
    var cv = document.getElementById('week-chart');
    if (!cv) return;
...
function renderSet() {
    ...
    if (nt) nt.checked = localStorage.getItem('notifications') === 'true';
}

document.getElementById('notif-toggle').addEventListener('change', function() {
    localStorage.setItem('notifications', this.checked);
    toast(this.checked ? 'Уведомления включены' : 'Уведомления выключены');
});
...
function exportData() {
    ...
    a.href = u; a.download = 'repeater_' + new Date().toISOString().slice(0, 10) + '.json'; a.click();
    ...
    r.onload = function(e) {
        try {
            ...
            imp.forEach(function(t) {
                t.id = genId(); t.dateAdded = t.dateAdded || Date.now(); t.nr = t.nr || Date.now();
                t.st = t.st || 0; t.interval = t.interval || 0; t.done = t.done || false; t.rc = t.rc || 0;
                topics.push(t);
            });
            save(); renderTopics(); renderDash();
            toast('Импортировано ' + imp.length + ' тем');
        } catch(e) { toast('Ошибка формата'); }
    };
    r.readAsText(f);
    ev.target.value = '';
}
...
function clearAllData() {
    if (confirm('Удалить все темы?')) {
        topics = []; save(); renderTopics(); renderDash(); toast('Данные удалены');
    }
}
